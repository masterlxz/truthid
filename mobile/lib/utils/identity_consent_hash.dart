...
Uint8List buildIdentityConsentHash({
  ...
  final encoded = BytesBuilder()
    ..add(uint256Bytes(chainId))
    ..add(addressBytes(identityRegistryAddress))
    ..add(uint256Bytes(BigInt.from(128)))
    ..add(addressBytes(controller))
    ..add(uint256Bytes(BigInt.from(usernameBytes.length)))
    ..add(usernameBytes)
