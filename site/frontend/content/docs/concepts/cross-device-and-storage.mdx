...
All three show an approval screen naming the requesting app before anything happens — same principle as the origin check during login (see [How TruthID Works → Logging in](/docs/concepts/how-it-works#logging-in)). Desktop additionally remembers a per-app authorization after the first approval, with a rolling daily quota (50 pins/day by default, configurable) so a trusted app doesn't need re-approval on every single request; the mobile cross-device path has no persistent quota — every request gets its own approval.
