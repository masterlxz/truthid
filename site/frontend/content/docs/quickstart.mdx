---
title: Quickstart
description: "Add Login with TruthID to your backend: create a challenge, show a QR code, and verify the signed response."
---
...
<Callout title="Session visibility — nothing to do" type="info">
Completed logins already show up in the user's TruthID mobile/desktop apps and can be individually revoked from there. The phone registers the session on-chain itself (via a UserOperation) before it ever calls your `/auth/verify` endpoint — there's no relayer to fund and no separate registration step. See the SDK reference ([TypeScript](/docs/sdk/typescript#verifysessionsessionhash), [Python](/docs/sdk/python#verify_sessionsession_hash), [Ruby](/docs/sdk/ruby#verify_sessionsession_hash)) if you want to double-check a session landed on-chain via `verifySession`.
</Callout>
...
- [How TruthID Works](/docs/concepts/how-it-works) — the full protocol behind what you just called
- [Desktop](/docs/apps/desktop) / [Mobile](/docs/apps/mobile) — what the "real device" in step 5 actually is
