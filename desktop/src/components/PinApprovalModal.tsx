    ...
    <div className="modal-overlay">
      <div className="modal-box">
        <div className="modal-header">
          <h2 className="modal-title">{t("pinApprovalModal.title")}</h2>
        </div>

        <div className="card">
          <p>
            <strong>{request.appName || t("pinApprovalModal.defaultAppName")}</strong>{" "}
            {t("pinApprovalModal.description")}
          </p>

          {expired && <p className="error-text">{t("pinApprovalModal.expired")}</p>}
          {error && <p className="error-text">{error}</p>}

          <div className="actions-row" style={{ marginTop: "0.75rem" }}>
            <button onClick={handleApprove} disabled={expired}>
              {t("pinApprovalModal.approve")}
            </button>
            <button onClick={handleReject} className="topbar-btn-danger">
              {t("pinApprovalModal.reject")}
            </button>
          </div>
        </div>
      </div>
