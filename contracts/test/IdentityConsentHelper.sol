...
abstract contract IdentityConsentHelper is Test {
    function _signConsent(IdentityRegistry registry, uint256 signerKey, string memory username, address controller)
        internal
        view
    ...
    function _createIdentity(IdentityRegistry registry, uint256 controllerKey, string memory username)
        internal
        returns (uint256 id)
    {
        address controller = vm.addr(controllerKey);
        (uint8 v, bytes32 r, bytes32 s) = _signConsent(registry, controllerKey, username, controller);
