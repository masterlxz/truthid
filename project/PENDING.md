...
> 
> Última atualização: 2026-08-19 (Sessão 212: removida UI de teste/debug antes de produção — botão "Test signature" do PasskeyBadge no Desktop e "Deep Link Self-Test" das Configurações no Mobile; registrado P57, landing page pública com instaladores fora da Chrome Web Store, ainda sem `/plan`)
...
| P57 | **Landing page pública com instaladores (Desktop + extensão sem Chrome Web Store)** — dono do projeto quer uma landing bonita pro app, com os instaladores do Desktop nativo e um pacote da extensão pra instalação manual, sem publicar na Chrome Web Store. Direção discutida (ainda sem `/plan`, nada implementado): landing nova no `site/frontend` (Next.js), publicada junto com a doc no mesmo GitHub Pages (estática, sem depender do backend Rails, mesmo padrão do deploy da doc — ver [[project-site]]); extensão distribuída como `.zip` da pasta buildada + passo a passo de `chrome://extensions` → "Modo desenvolvedor" → "Carregar sem compactação" (mesmo fluxo já usado nos testes E2E, P30/P33) — é o único caminho viável hoje pra instalar fora da Web Store, já que Chrome/Brave bloqueiam `.crx` auto-atualizável pra usuário comum fora de política de empresa. Falta decidir: conteúdo/design da página, onde hospedar os binários dos instaladores do Desktop (GitHub Releases é o candidato óbvio), se builda os instaladores agora ou só quando a página estiver pronta. | conversa direta (Sessão 212) | 🟡 Baixa |
