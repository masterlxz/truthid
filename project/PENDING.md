...
> 
> Última atualização: 2026-08-01 (Sessão 181 — P37 parcialmente validado: documentos pinam separado, confirmados via gateway público; achado bug real P46, botões de documento não respondem a clique)
...
| P46 | **Botões "Baixar" e ⭐ (favoritar) não respondem a clique em entradas tipo documento** — achado real ao tentar validar o P37 (leitura offline). `handleDownloadDocument`/`handleToggleFavorite` em `VaultManagement.tsx` não disparam nenhum efeito visível ao clicar (nenhum diálogo `save()` abre, nenhum estado muda) — reproduzido de forma consistente, inclusive depois de reiniciar o processo do zero (descarta corrupção de estado da sessão). Contraste que isola o problema: o `save()` do Backup export (`useVaultBackup.ts`) funciona normalmente na mesma janela, e cliques fora do card do documento (ex: accordion "Gerenciar perfis") respondem normalmente — o problema é específico dos botões de ação dentro do card de entradas tipo documento. Causa raiz não identificada (sem acesso a devtools neste ambiente pra ver o erro/exceção real — `save()` do plugin é simples o bastante que não deveria falhar sozinho). O processo do Desktop chegou a fechar sozinho sem panic no log durante a investigação (possível esgotamento de recurso após muitos diálogos GTK abertos na sessão) — reiniciado sem perda de dados (Vault v11 confirmado intacto on-chain). | `SESSIONS.md` (Sessão 181, achado ao validar P37) | 🟠 Média |
