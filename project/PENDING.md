...
| P49 | **Internacionalização (i18n) — seleção de idioma no software e na documentação** — pedido explícito do dono do projeto (Sessão 202). Hoje não existe nenhuma camada de i18n/tradução nem seletor de idioma em nenhuma frente (Desktop, Mobile, extensão, site de doc em `site/frontend`) — confirmado por busca (`i18n`/`intl`/`localization`/`LanguageSelector`), zero ocorrência fora de artefatos de build irrelevantes. Escopo, idiomas-alvo e ordem de prioridade entre software vs. doc ainda não definidos com o dono do projeto; nenhum `/plan` rodado. | conversa direta (Sessão 202) | 🟡 Baixa |
