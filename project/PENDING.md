...
> 
> Última atualização: 2026-07-27 (Sessão 175 — P8/Fase 15: 15.6 fatia 1 (Autofill SO iOS, scaffold + registro de identidades) concluída)
...
| P35 | **Build real — target `AutofillExtension` (15.6, fatia 1)** — o target foi criado programaticamente (gem `xcodeproj`) e validado por inspeção (dependency, embed phase, bundle id, entitlements, configs Debug/Release/Profile), mas nunca compilado — este ambiente é Linux, sem Xcode/macOS/simulador. Falta abrir `Runner.xcodeproj` num Mac, confirmar que builda sem erro, habilitar "TruthID" em Ajustes → Senhas → Preencher senhas automaticamente, e confirmar que as identidades registradas via `ASCredentialIdentityStore` aparecem na lista/QuickType bar. | `PHASE.md` (Fase 15.6, fatia 1) | 🟠 Média |

### Funcionalidades Não Implementadas

| ID | Item | Onde se originou | Prioridade |
|---|---|---|---|
| P8 | **Phase 15 — Digital Identity Vault** — documentos, endereços, cartões de crédito. 8 etapas planejadas. **15.1-15.5 concluídas** (15.1 schema S167, 15.2 CRUD Desktop S168, 15.3 CRUD Mobile S169, 15.4 fatia 1 endereço/LAN/Mobile S170 + fatia 2 cartão/LAN/Mobile S171 + dead-drop S172 + Desktop S173 — 15.4 fechada por completo —, 15.5 Autofill SO Android S174), **15.6 fatia 1 concluída** (scaffold + registro de identidades, S175) — faltam 15.6 fatia 2 (ver P36), 15.7, 15.8. | `PHASE.md` (Fase 15) | 🟠 Média |
| P9 | **Phase 15 — Autofill SO (Android/iOS)** — implementar `AutofillService` e `ASCredentialProviderViewController`. **Android (15.5) concluído na Sessão 174** — `TruthIdAutofillService.kt` detecta campos e delega a leitura/aprovação pra Activity Flutter existente (mesmo Vault/AppLockGate/picker da Fase 15.4), sem duplicar acesso ao vault em Kotlin. **iOS (15.6) fatia 1 concluída na Sessão 175** — target `AutofillExtension` + registro de identidades no `ASCredentialIdentityStore` (só credencial — endereço/cartão não têm extension point público no iOS, achado real da sessão). Falta a fatia 2 (decifra do Vault dentro da extensão, ver P36) e validar a fatia 1 num Mac real (P35). | `PHASE.md` (Fase 15, etapas 15.5/15.6) | 🟠 Média |
...
| P36 | **15.6, fatia 2 — decifra do Vault dentro da extensão iOS** — `CredentialProviderViewController` hoje só mostra uma tela informativa e cancela. Preencher de verdade exige portar a cripto do Vault (AES-GCM/HKDF) pra Swift, rodando dentro do processo da extensão (achado da Sessão 175: diferente do Android, a extensão iOS não pode abrir o app principal e receber o resultado de volta — `extensionContext.open(_:)` faz a extensão perder o controle da chamada original). Precisa de Keychain Access Group + App Group compartilhados entre `Runner` e `AutofillExtension`. Trabalho grande, não iniciado. | `PHASE.md` (Fase 15.6, fatia 1) | 🟠 Média |
