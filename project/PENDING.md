...
> 
> Última atualização: 2026-08-14 (Sessão 200: doc migrada pra `/docs` via Fumadocs; decisão de sequenciamento — doc primeiro, monetização segue em dev sem lançamento ao vivo por um bom tempo, P47 atualizado)
...
| P47 | **`site/` — próximas fatias, fora do escopo do v1** — billing (Stripe/Mercado Pago) + Entitlement Service, bootstrap de identidade TruthID sem Ledger (item 2.1 do épico de tier facilitado), GitHub e o próprio TruthID como provider adicional de login (modelo `Identity` já suporta). Migração de `docs/` (Docusaurus) pro site **começou na Sessão 200** (Fumadocs, `/docs` no `site/frontend`, 10 páginas com paridade total). Billing/Entitlement/bootstrap sem Ledger seguem em desenvolvimento, mas **decisão explícita do dono do projeto (Sessão 200): sem lançamento ao vivo por um bom tempo** — prioridade atual é continuar a doc. Nenhuma dessas fatias tem `/plan` rodado ainda. | `SESSIONS.md`/`ROADMAP.md` (Sessão 199, 200) | 🟡 Baixa |
