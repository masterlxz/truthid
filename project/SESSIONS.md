...
> **Nota de migração (Sessão 151, 2026-07-24)**: este log de sessões foi extraído do antigo `PROJECT_STATE.md` (arquivo monolítico na raiz), que foi dividido em arquivos menores dentro de `project/`. As referências a `PROJECT_STATE.md` neste log são históricas — refletem o nome do arquivo no momento em que a sessão foi registrada. Para o estado atual do projeto, veja `project/INDEX.md`.
