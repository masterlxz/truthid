...
| 3 | `DeviceRegistry` (débito #34) | `contracts/src/DeviceRegistry.sol` | ver Fase 1.6 | Redeploy em Sepolia + Mainnet — novo parâmetro `encryptedVaultKey` em `registerDevice`, novo mapping `deviceVaultKeys`, evento `DeviceRegistered` expandido. Após redeploy, atualizar `DEVICE_REGISTRY_ADDRESS` + `DEVICE_REGISTRY_DEPLOY_BLOCK` em `desktop/src/config/contracts.ts`, `mobile/lib/services/blockchain_service.dart`, SDKs e docs. | Sessão 76 / débito #34 |
