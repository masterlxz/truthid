...
**Pendências**:
- Fatia 2b (extensão consome o dead-drop: derivar o mesmo nome IPNS em TS, poll contra gateway público `/ipns/<name>`, decifrar, UI de progresso) — não iniciada.
...
- Endpoint `/ipns/<name>` em gateway público (pro polling da extensão, fatia 2b) não foi investigado — comportamento de cache do gateway pode mascarar atualizações recentes.
...
- [~] 13.9 — Extensão de navegador: sessão efêmera, autofill, revogação em cascata *(Sessão 99 — **fatia 1 implementada: só transporte LAN** (revogação confirmada como TTL curto sem canal ativo); Sessão 100 — **fatia 2a implementada: Mobile publica o dead-drop IPFS/IPNS**, validado contra Kubo real. Falta fatia 2b (extensão consome). Ver seção "Extensão de navegador (13.9)" abaixo para o desenho completo, achados e pendências)*
