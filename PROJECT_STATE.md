...
**Pendências**:
- Fatia 2 (dead-drop IPFS/IPNS) — não iniciada, é a próxima etapa da 13.9.
...
- [~] 13.9 — Extensão de navegador: sessão efêmera, autofill, revogação em cascata *(Sessão 99 — **fatia 1 implementada: só transporte LAN**, escopo confirmado com o dono do projeto (dead-drop IPFS/IPNS fica pra uma fatia 2; revogação confirmada como TTL curto sem canal ativo). Ver seção "Extensão de navegador (13.9)" abaixo para o desenho completo, achados e pendências)*
