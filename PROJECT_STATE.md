...
| 17 | `contracts/src/IdentityRegistry.sol:80` | `createIdentity(username, controller)` não verificava se `msg.sender` tinha qualquer autorização sobre o `controller` informado. Achado (CONFIRMED) no `/code-review` da Sessão 53. Permitia squatting/griefing: qualquer um podia "ocupar" um endereço alheio (inclusive o CREATE2 pré-computado de uma smart account que ainda vai ser deployada) chamando `createIdentity` primeiro. | **IMPLEMENTADO e testado em Sepolia — Sessão 62, opção (a)**: `createIdentity` agora exige assinatura de consentimento (v,r,s) — do próprio controller (EOA) ou do owner via `factory.getAddress(signer)` (smart account pré-deploy). Ver Log de Sessões, Sessão 62, para o desenho completo e o teste end-to-end (incluindo um bug de gas real encontrado e corrigido no funding da smart account). **Deploy em Base Mainnet ainda pendente** (ver "Pendências de Deploy" abaixo) — só fecha de vez quando o redeploy acontecer. |
...
| 3 | `IDENTITY_REGISTRY_ADDRESS`, `DEVICE_REGISTRY_ADDRESS`, `SESSION_REGISTRY_ADDRESS`, `RECOVERY_MANAGER` (em `FACTORY_IMMUTABLES`), `TRUTHID_ACCOUNT_FACTORY_ADDRESS` | `desktop/src/config/contracts.ts` e `truthidAccount.ts` | Endereços atuais (pré-débito #17) | ✅ Sepolia redeployado e testado (Sessão 62 — endereços registrados no Log de Sessões). **Mainnet ainda pendente** — requer Ledger físico. Config do desktop continua apontando pros endereços antigos até o redeploy da mainnet acontecer. **Atenção**: existe 1 identidade real na mainnet atual que será perdida — dono do projeto já decidiu aceitar a perda e recriar manualmente depois. | Débito #17 |
...
**Deploy em Base Mainnet continua pendente** — dono do projeto decidiu fechar a sessão por aqui, sem redeployar mainnet ainda. Passos 4-7 da lista "PENDENTE" acima continuam de pé (mainnet + atualizar endereços no desktop + recriar identidade mainnet perdida + marcar débito #17 resolvido).
...
- **Resultado**: implementação completa e verificada (contratos + Rust + desktop), commitada. Sepolia deployado e testado de ponta a ponta (achando e corrigindo o bug de gas do funding no processo). Mainnet, explicação detalhada pro usuário, e a tela de extrato ficaram pendentes.
- **Próximo passo**: explicar a implementação em detalhe pro usuário; depois, deploy em Base Mainnet (passos 4-7 da lista "PENDENTE"); depois, desenhar a tela de extrato da smart account.
