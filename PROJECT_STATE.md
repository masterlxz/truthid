...
> Última atualização: 2026-07-04 (Sessão 69 — auditoria on-chain + redeploy da factory em Sepolia; correção de inconsistências deste arquivo)
>
...
| 33 | `desktop/src/components/VaultManagement.tsx` (fluxo "Enviar"), `contracts/src/VaultRegistry.sol:71` | `VaultRegistry.updateVault` só aceita chamada de quem `IdentityRegistry.getUsernameByController(msg.sender)` resolve como controller da identidade. `VaultManagement.tsx` (escrito na Sessão 51, **antes** da Fase 14 existir) dispara `writeContract({... functionName: "updateVault" ...})` **direto pela wallet conectada** (a Ledger/EOA), em vez de rotear via `TruthIDAccount.execute(VAULT_REGISTRY_ADDRESS, 0, calldata)` contra o `smartAccountAddress` — mesma classe de bug já corrigida no débito relacionado da 14.8 (Sessão 63) para pareamento/revogação de device. Prova: o componente só desestrutura `identityId` de `useIdentity()`, nunca `smartAccountAddress` (que já existe no `IdentityContext` e é usado em `SmartAccountDashboard`/`WithdrawModal`). **Não quebrou nada ainda** porque `VaultRegistry` nunca foi deployado (`VAULT_REGISTRY_ADDRESS` continua o endereço zero placeholder) — mas vai reverter pra qualquer identidade smart-account assim que alguém deployar o contrato e clicar em "Enviar". Achado numa revisão manual pontual (não via `/code-review`), motivada por uma pergunta de acompanhamento depois da Fase 14 sobre revisar a Fase 13 em busca de inconsistências. | Trocar o `writeContract` direto por uma chamada envolta em `execute()` (endereço destino = `smartAccountAddress`, `dest` = `VAULT_REGISTRY_ADDRESS`, `value` = 0, `func` = calldata de `updateVault` via `encodeFunctionData`), mesmo padrão do `WithdrawModal.tsx` (Sessão 71). Vale também conferir o restante do fluxo do Vault (13.1–13.7) em busca de outras chamadas on-chain que ainda assumam o controller = EOA em vez de smart account, antes de destravar 13.8/13.9 — checagem pendente, ver Log de Sessões. |
| 34 | `mobile/lib/services/vault_key_service.dart:23` | A chave AES do vault é derivada da chave privada do próprio device (`DeviceKeyService.getPrivateKeyBytes()` via HKDF), não de um segredo compartilhado da identidade — mesmo padrão em `desktop/src-tauri/src/lib.rs` (`derive_vault_key()`). Isso contradiz o design documentado (linha ~708 deste arquivo): "o vault é cifrado com uma chave simétrica própria do vault... compartilhada entre os devices do usuário apenas no momento do pareamento". Nenhum código (`PairDevice.tsx`, mobile, desktop) implementa esse compartilhamento/wrapping de chave hoje. Achado no `/code-review high` da Sessão 75 (escopo: arquivos do Vault). | Verificado (CONFIRMED). Implementar o compartilhamento de chave no momento do pareamento (ex: o device já pareado envia a chave do vault cifrada pro device novo via o mesmo canal usado pra parear), ou redesenhar a derivação pra depender de um segredo ao nível de identidade em vez de por-device. Decisão de arquitetura pendente do dono do projeto — bloqueia qualquer sincronização real entre 2+ devices do Vault. |
| 35 | `desktop/src/components/VaultManagement.tsx:386` | `handleTogglePerm` chama `invoke("vault_set_device_permission", { pub_key, can_write })` com chaves snake_case, mas o comando Rust (`fn vault_set_device_permission(pub_key: String, can_write: bool)`, sem `rename_all`) espera as chaves JS em camelCase (`pubKey`/`canWrite`) — mesma convenção já usada em outras chamadas funcionais do próprio arquivo (ex: `get_ledger_address` com `accountIndex`). O toggle "Pode escrever"/"Só leitura" por device nunca funcionou; o erro é engolido por um `catch` vazio. Achado no `/code-review high` da Sessão 75. | Verificado (CONFIRMED). Trocar `{ pub_key: pubKey, can_write: canWrite }` por `{ pubKey, canWrite }`. Também vale remover o `catch` vazio ou pelo menos logar/mostrar o erro, pra esse tipo de bug não ficar invisível de novo. |
| 36 | `desktop/src/components/VaultManagement.tsx:317` | A condição que decide sucesso/erro após `vault_publish` só lança erro quando **todos** os provedores de pin falham (`providers_failed.length > 0 && providers_ok.length === 0`). Falha parcial (alguns provedores ok, outros não) é tratada como sucesso total — o `updateVault` prossegue on-chain sem avisar que a redundância de pinning foi perdida. Achado no `/code-review high` da Sessão 75. | Verificado (CONFIRMED). Mostrar aviso (não bloqueante) quando `providers_failed` não estiver vazio, mesmo com `providers_ok` não-vazio — usuário precisa saber que nem todo provedor configurado realmente tem o blob. |
| 37 | `desktop/src/components/VaultSettings.tsx:70` | `healthStatus` (resultado do health-check por provedor) é indexado pela posição no array `providers`. `handleRemove` apaga só a chave do índice removido, sem reindexar os provedores seguintes — depois de remover um provedor do meio da lista, o status de saúde exibido fica associado ao provedor errado. Achado no `/code-review high` da Sessão 75. | Verificado (CONFIRMED). Indexar `healthStatus` por um identificador estável do provedor (ex: `endpoint_url` ou um id gerado), não pela posição no array; ou simplesmente limpar `healthStatus` inteiro a cada remoção, forçando um novo health-check. |
| 38 | `mobile/lib/services/vault_repository.dart:155` | `updateEntry` não verifica se encontrou uma entrada com o id informado antes de salvar — um id inexistente/obsoleto vira um no-op silencioso que ainda assim incrementa `version` e devolve a entrada como se tivesse sido atualizada. A implementação Rust irmã (`desktop/src-tauri/src/vault.rs`, `upsert`) trata esse caso inserindo como nova entrada; o port Dart descartou esse tratamento. Achado no `/code-review high` da Sessão 75. | Verificado (CONFIRMED). Fazer `updateEntry` lançar (ou retornar null/Result) quando nenhuma entrada correspondente for encontrada, em vez de reportar sucesso silencioso — ou replicar o comportamento do Rust (inserir como nova) se for esse o comportamento pretendido. |
| 39 | `desktop/src/components/VaultManagement.tsx:288` | O `useEffect` que dispara `updateVault` depois do `vault_publish` só depende de `[pendingUpdate]`. Se a wallet não estiver conectada quando o efeito roda, ele abre o modal de conexão e retorna sem chamar `writeContract` — mas como `isConnected` não está nas dependências, conectar a wallet depois nunca reexecuta o efeito. Achado no `/code-review high` da Sessão 75. | Verificado (CONFIRMED, tem contorno manual — clicar "Enviar" de novo funciona). Adicionar `isConnected` (ou um efeito separado ouvindo essa transição) às dependências, ou mover a checagem de conexão para antes de setar `pendingUpdate`. |
| 40 | `desktop/src/components/VaultSettings.tsx:90` | `handleFormAdd` só exige `name`/`endpoint_url` preenchidos, mesmo quando `kind === "psa"` — a API key (obrigatória pra qualquer provedor PSA funcionar) não tem validação equivalente antes de salvar. Achado no `/code-review high` da Sessão 75. | Verificado (CONFIRMED, severidade baixa). Exigir `api_key` não-vazio quando `kind === "psa"`, mesmo padrão de validação condicional já usado pra outros campos do formulário. |
| 41 | `contracts/src/VaultRegistry.sol:71` | `updateVault` valida que `cid` não é vazio mas nunca valida que `contentHash` é diferente de zero, apesar do comentário do struct dizer que esse campo existe pra verificação de integridade. Achado no `/code-review high` da Sessão 75. | Verificado (CONFIRMED, severidade baixa — nenhuma lógica on-chain lê `contentHash` hoje, a verificação é feita pelos clients off-chain). Adicionar `if (contentHash == bytes32(0)) revert EmptyContentHash();`, mesmo padrão do `EmptyCid()` já existente. Baixa prioridade — não é urgente, mas é barato de corrigir. |
