...
| 17 | `contracts/src/IdentityRegistry.sol:80` | `createIdentity(username, controller)` não verificava se `msg.sender` tinha qualquer autorização sobre o `controller` informado. Achado (CONFIRMED) no `/code-review` da Sessão 53. Permitia squatting/griefing: qualquer um podia "ocupar" um endereço alheio (inclusive o CREATE2 pré-computado de uma smart account que ainda vai ser deployada) chamando `createIdentity` primeiro. | **IMPLEMENTADO (código) — Sessão 62, opção (a)**: `createIdentity` agora exige assinatura de consentimento (v,r,s) — do próprio controller (EOA) ou do owner via `factory.getAddress(signer)` (smart account pré-deploy). Ver Log de Sessões, Sessão 62, para o desenho completo. **Deploy ainda pendente** (cascata de 5 contratos em Sepolia + Mainnet, ver "Pendências de Deploy" abaixo) — só fecha de vez quando o redeploy acontecer. |
...
| 3 | `IDENTITY_REGISTRY_ADDRESS`, `DEVICE_REGISTRY_ADDRESS`, `SESSION_REGISTRY_ADDRESS`, `RECOVERY_MANAGER` (em `FACTORY_IMMUTABLES`), `TRUTHID_ACCOUNT_FACTORY_ADDRESS` | `desktop/src/config/contracts.ts` e `truthidAccount.ts` | Endereços atuais (pré-débito #17) | **Pendente** — redeploy em cascata dos 5 contratos (Sepolia + Mainnet) por causa da mudança de assinatura do `createIdentity` (débito #17, Sessão 62). Requer Ledger físico. Ver Log de Sessões, Sessão 62, pra lista de passos e comandos. **Atenção**: existe 1 identidade real na mainnet atual que será perdida — dono do projeto já decidiu aceitar a perda e recriar manualmente depois. | Débito #17 |
...
- **Resultado**: implementação completa e verificada (contratos + Rust + desktop), commitada. Deploy e explicação detalhada ficaram pendentes.
- **Próximo passo**: ver lista "PENDENTE" acima.
