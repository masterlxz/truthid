...
## 5. Register the session on-chain *(optional)*
...
```python
# Extend your /auth/verify handler — after verify_auth_response succeeds:
session_signature = data.get("sessionSignature")
if session_signature and os.environ.get("RELAYER_PRIVATE_KEY"):
    try:
        truthid.register_session(
            nonce=challenge.nonce,
            identity_id=result.identity_id,
            device_pub_key=result.device_address,
...
```ruby
# Extend your /auth/verify handler — after verify_auth_response succeeds:
session_signature = data["sessionSignature"]
...
See the SDK reference for full details: [TypeScript](/docs/sdk/typescript#registersessionparams), [Python](/docs/sdk/python#register_session), [Ruby](/docs/sdk/ruby#register_session).
