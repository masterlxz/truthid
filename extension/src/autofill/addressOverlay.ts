  ...
  async function openPanel(): Promise<void> {
    ...
    try {
      permission = await chrome.runtime.sendMessage({
        type: AUTOFILL_ADDRESS_ENSURE_HOST_PERMISSION_MESSAGE,
      });
    } catch {
    ...
    void chrome.runtime
      .sendMessage({ type: AUTOFILL_ADDRESS_SWEEP_MESSAGE, sessionId })
      .then((response) => {
    ...
    connectButton.addEventListener('click', () => {
      ...
      void chrome.runtime
        .sendMessage({
          type: AUTOFILL_ADDRESS_MANUAL_FETCH_MESSAGE,
          host: manualHost,
          sessionId,
        })
        .then((response) => {
