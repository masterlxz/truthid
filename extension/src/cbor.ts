 ...
 * of scope on purpose. Porte byte-a-byte de `desktop/src/utils/cbor.ts`
 * (Sessão 134, criação de passkey via extensão) — mesmo padrão de
 * duplicação por plataforma já estabelecido no projeto.
 */
...
export function encodeMap(entries: Array<[Uint8Array, Uint8Array]>): Uint8Array {
  ...
  const chunks: Uint8Array[] = [new Uint8Array(head)];
